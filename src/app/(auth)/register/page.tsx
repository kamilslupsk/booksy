...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (password.length < 8) { toast.error("Hasło musi mieć co najmniej 8 znaków"); return; }
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ email, password, displayName, category }),
      });
      ...
      if (result?.error) {
        toast.error("Konto utworzone — zaloguj się ręcznie");
        router.push("/login");
        return;
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 py-12">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-200">
          ...
          <p className="text-sm text-slate-500 mb-6">Zacznij przyjmować rezerwacje. Bez NIP, bez umów.</p>
